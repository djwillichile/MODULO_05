...
Los archivos de tipo monobanda, como ya se mencionó anteriormente, corresponden a archivos que solo contienen un raster en su arquitectura, por lo tanto es posible importarlos mediante la función `raster()` del paquete homónimo, siendo éste, capaz de reconocer qué extensión tienen los archivos y configurar la imagen de acuerdo a ésto. A continuación se observa el procedimiento standar para importar archivos monobanda.
...
Por otra parte, los archivos de tipo multibanda, corresponden a archivos que, en su arquitectura, pueden contener más de na imagen raster, por lo tanto, poseen un sistema interno algo más complejo generando una arquitectura conocida como pila o stack, es así que, para importar archivos multibanda se utilizan las funciones `brick()` y `stack()` provenientes ambas del paquete `raster`, siendo éste, capaz también de reconocer qué extensión tienen los archivos y configurar las imágenes de acuerdo a ésto. A continuación se observa el procedimiento standar para importar archivos multibanda.
...
## álgebra de mapas
El álgebra de mapas incluye un amplio conjunto de operadores que se ejecutan sobre una o varias capas raster de entrada para producir una o varias capas raster de salida. Por operador se entiende un algoritmo que realiza una misma operación en todas las celdillas de una capa raster. Estos operadores se definen mediante ecuaciones, por ejemplo el operador B = A∗100 genera una nueva capa (B) asignando a cada celdilla el valor de la celdilla correspondiente en la capa (A) multiplicado por 100.
Cada capa raster es una matriz de números y la operación se realiza para todos los números de la matriz, por tanto para todas las celdillas de la capa raster. Sin embargo aunque ambas operan sobre matrices, el álgebra de mapas tiene poco más que ver con el concepto matemático de álgebra matricial.

Los operadores aritméticos y lógicos utilizados para el álgebra de mapas coincide justamente con los operadores aritméticos y lógicos presentados en el módulo 2 de este curso por lo que no nos detendremos a detallarnos pero si haremos incapié en su ejecución, para poder obtener productos en función de los materiales en formato raster.

Una aplicación muy extendida del álgebra de mapas en el mundo de los Sistemas de Información Geográfica y la Teledetección el el cálculo de índices espectrales. En concreo ahora realizaremos 2 índices, el Índice de Nieve de Diferencia Normalizada (NDSI) y el Índice de Vegetación de Diferencia Normalizada (NDVI)
...
pTemp=brewer.pal(n = 11, name = "Spectral")
pAlt=brewer.pal(n = 11, name = "BrBG")
...
<div clases="footer">Copyright &copy; Guillermo S. Fuentes Jaque 2021</div>